"""
Copyright 2025 A R Shakeel Irfan, V S Rohit, S Tharun,
...
# zxcvbn is used for realistic password strength scoring. Attempt to import and provide
# a helpful error message if it's missing so users know how to install it.
try:
    from pyzxcvbn import zxcvbn
except ImportError:
    print("Error: required module 'pyzxcvbn' is not installed.")
    print("Please install dependencies with: pip install pyzxcvbn")
    sys.exit(1)
...
def _tk_supported() -> bool:
    """Return True if a Tkinter GUI can be safely used.

    On macOS older Tk/Tcl builds may abort at runtime when trying to create
    a root window.  The embedded check here uses the Darwin version number
    (platform.release) which corresponds to the kernel version; the error
    message observed in CI was:

        macOS 26 (2603) or later required, have instead 16 (1603) !

    Darwin 26 is macOS 14.0 (Somona) and Darwin 16 is macOS 10.12.  The
    crash happens before Python can catch an exception, so we avoid ever
    creating a Tk root on older Darwin kernels and fall back to the CLI.
    """
    if platform.system() != "Darwin":
        # non-macOS platforms typically don't exhibit this issue
        return True
    try:
        darwin_major = int(platform.release().split(".")[0])
    except (ValueError, IndexError):
        return True
    # require Darwin 26+ which corresponds roughly to macOS 14.0
    return darwin_major >= 26
...
def main():
    ...
    # decide whether we will run CLI logic; this covers explicit CLI flags
    # and situations where the GUI library is known to be unsafe on this host.
    use_cli = bool(args.cli or args.check or args.generate or not _tk_supported())

    if use_cli:
        if not _tk_supported() and not (args.cli or args.check or args.generate):
            # user didn't ask for CLI but GUI is not supported
            print(
                "<Warning> Tkinter GUI is not supported on this macOS version.\n"
